...
<<INB for single-time CT lung-cancer screen>>=
# Effectiveness and costs from Bill Black's NEJM paper
LYG <- (8.4792-6.8479)/3 # = e_1-e_2
treatmentcostdiff <- 28195-24192 #27385-25379 # = c_1-c_2
dollarsperLYG <- 1e5 #1.53176375e5
e <- LYG*dollarsperLYG - treatmentcostdiff # = net effectiveness of early intervention
c0 <- 500 # = c_0, cost of definitive test for lung cancer
cm <- 404+298 # = c_m, cost of single CT screen plus workup cost
piINB <- c0/(e+c0) # = cost-effective optimal risk-threshold

# CT screen characteristics from Tim Church's NEJM paper
sens <-273/292 ; pi <- 292/26382 ; p <- 7212/26382 ; PPV <- 273/7212 ; truepos <- sens*pi ; 
spec <- 19151/26090 ; NPV <- spec*(1-pi)/(1-p)

# Calculate INB and critical dollars per LYG to have positive INB
INB <- (e+c0)*truepos - cm - c0*p
dollarsperLYG.critical1 <- ( (cm + c0*p)/truepos - c0 + treatmentcostdiff ) / LYG
print(paste("INB = $",INB,", minimum dollars per LYG to have positive INB = $",dollarsperLYG.critical1))
dollarsperLYG.critical2 <- ( c0*(1-p)/truepos-cm + treatmentcostdiff ) / LYG
print(paste("INB = $",INB,", maximum dollars per LYG before we should test everyone = $",dollarsperLYG.critical2))
print(paste("cost-effective optimal risk-threshold = ",piINB))
print(paste("net effectiveness of early intervention e = $",e))

# Compare NLST criteria to Lung-RADS
# Use Paul Pinsky's Table 4 for numbers, which is not the NLST paper, but this is not a self-consistent 2x2 table
# I'm going to keep the margins for NLST and Lung-RADS fixed as the denominators in Table 4
# For the interior of the table, I will keep the D+,M+ cell as he reports (248 for Lung-RADS,), and calculate the rest
# Lung-RADS:    M+    M-
#          D+  248    44    292
#          D- 3343 22747  26090
#             3591 22791  26382
# For NLST, I keep the interior (273,19,6939) and n=26382.  The M- margin is forced to be 19170 and thus D-,M- must be 19151
# NLST:    M+    M-
#     D+  273    19    292
#     D- 6939 19151  26090
#        7212 19170  26382
NLSTsens <- 273/292 ; NLSTspec <- 19151/26090 ; NLSTPPV <- 273/7212 ; NLSTpi <- 292/26382 ; NLSTp <- 7212/26382  
RADSsens <- 248/292 ; RADSspec <- 22747/26090 ; RADSPPV <- 248/3591 ; RADSpi <- 292/26382 ; RADSp <- 3591/26382  

NLST.INB <- (e+c0)*NLSTsens*NLSTpi - cm - c0*NLSTp
RADS.INB <- (e+c0)*RADSsens*RADSpi - cm - c0*RADSp
print(paste("INB for NLST vs Lung-RADS is:",NLST.INB,RADS.INB, ", difference=",NLST.INB-RADS.INB))

# criteria for better performance; use the specificity version since Lung-RADS has better specificity
Lorenz.slope <- (RADSspec-NLSTspec) / ( (1-RADSp) - (1-NLSTp) )
critical <- (1-piINB)/(1-NLSTpi) # NLSTpi=RADSpi so we have consistency
print(paste("Lorenz.slope =",Lorenz.slope,"must be greater than",critical,"for Lung-RADS to have better INB than NLST"))
# Same critera but on the sensitivity scale, so NLST is the test with better sensitivity
Lorenz.slope <- (NLSTsens-RADSsens) / (NLSTp-RADSp)
critical <- piINB/NLSTpi # NLSTpi=RADSpi so we have consistency
print(paste("Lorenz.slope =",Lorenz.slope,"must be greater than",critical,"for NLST to have better INB than Lung-RADS"))
@
